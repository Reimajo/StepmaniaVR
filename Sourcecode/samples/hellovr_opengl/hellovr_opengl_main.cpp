...
//-----------------------------------------------------------------------------
// Purpose: Main Application class with function defs
//------------------------------------------------------------------------------
...
public:
	...
	void printDevicePositionalData(int index, const char * deviceName, vr::HmdMatrix34_t posMatrix, vr::HmdVector3_t position, vr::HmdQuaternion_t quaternion);
	void startStepMania();
	void keyDown(WORD keycode);
	void keyUp(WORD keycode);
	int npoly(int nvert, float * vertx, float * verty, float testx, float testy);
...
//-----------------------------------------------------------------------------
// Purpose: Uses the position to calculate if a stepmania button should be pressed
//-----------------------------------------------------------------------------
void CMainApplication::printDevicePositionalData(int index, const char * deviceName, vr::HmdMatrix34_t posMatrix, vr::HmdVector3_t position, vr::HmdQuaternion_t quaternion)
{
	...
	static float p22_X = 0;
	static float p11_X = 0;
	static float p12_X = 0;
	static float p13_X = 0;
	static float p14_X = 0;
	static float p23_X = 0;
	static float p32_X = 0;
	static float p41_X = 0;
	static float p21_X = 0;
	static float p31_X = 0;
	static float p42_X = 0;
	static float p43_X = 0;
	static float p34_X = 0;
	static float p24_X = 0;
	static float p44_Y = 0;
	static float p33_Y = 0;
	static float p22_Y = 0;
	static float p11_Y = 0;
	static float p12_Y = 0;
	static float p13_Y = 0;
	static float p14_Y = 0;
	static float p23_Y = 0;
	static float p32_Y = 0;
	static float p41_Y = 0;
	static float p21_Y = 0;
	static float p31_Y = 0;
	static float p42_Y = 0;
	static float p43_Y = 0;
	static float p34_Y = 0;
	static float p24_Y = 0;

	static int indexTracker1 = 0;
	...
	if (zeitNachStart < 10) {
		...
		if (punkteSchonBerechnetBool) {
			...
			if (position.v[1] - bodenhoeheY < 0.03f) {
				...
				if (index == indexTracker2) {
					const WORD ARROW_UP = VK_UP;
					const WORD ARROW_DOWN = VK_DOWN;
					const WORD ARROW_LEFT = VK_LEFT;
					const WORD ARROW_RIGHT = VK_RIGHT;
					boolean atLeastOneKeyPressed = false;
					// ------------------- Arrow UP ---------------------------------
					static bool arrowUpPressed = false;
					if (arrowUpPressed == false && arrowUpHit == true) {
						dprintf("\nArrowUp -> PRESS");
						arrowUpPressed = true;
						keyDown(ARROW_UP);
						atLeastOneKeyPressed = true;
					}
					else if (arrowUpPressed == true && arrowUpHit == false) {
						dprintf("\nArrowUp -> RELEASE");
						keyUp(ARROW_UP);
					...
					// ------------------- Arrow DOWN ---------------------------------
					static bool arrowDownPressed = false;
					if (arrowDownPressed == false && arrowDownHit == true) {
						dprintf("\nArrowDown -> PRESS");
						arrowDownPressed = true;
						keyDown(ARROW_DOWN);
						atLeastOneKeyPressed = true;
					}
					else if (arrowDownPressed == true && arrowDownHit == false) {
						dprintf("\nArrowDown -> RELEASE");
						keyUp(ARROW_DOWN);
			...
			p41_X = (p44_X + (1.0f / 2.0f* (p11_X - (p44_X)))) - ((p44_Y - (p44_Y + (1.0f / 2.0f* (p11_Y - (p44_Y)))))*(-1));
			p41_Y = (p44_Y + (1.0f / 2.0f* (p11_Y - (p44_Y)))) - (p44_X - (p44_X + (1.0f / 2.0f* (p11_X - (p44_X)))));

			p33_X = p44_X + (1.0f / 3.0f * (p11_X - (p44_X)));
			p22_X = p44_X + (2.0f / 3.0f * (p11_X - (p44_X)));
			p12_X = (p11_X + (1.0f / 3.0f * (p14_X - (p11_X))));
			p13_X = (p11_X + (2.0f / 3.0f * (p14_X - (p11_X))));
			p23_X = (p14_X + (1.0f / 3.0f * (p41_X - (p14_X))));
			p32_X = (p14_X + (2.0f / 3.0f * (p41_X - (p14_X))));
			p21_X = (p11_X + (1.0f / 3.0f * (p41_X - (p11_X))));
			p31_X = (p11_X + (2.0f / 3.0f * (p41_X - (p11_X))));
			p42_X = (p44_X + (1.0f / 3.0f * (p41_X - (p44_X))));
			p43_X = (p44_X + (2.0f / 3.0f * (p41_X - (p44_X))));
			p34_X = (p44_X + (1.0f / 3.0f * (p14_X - (p44_X))));
			p24_X = (p44_X + (2.0f / 3.0f * (p14_X - (p44_X))));

			p33_Y = p44_Y + (1.0f / 3.0f * (p11_Y - (p44_Y)));
			p22_Y = p44_Y + (2.0f / 3.0f * (p11_Y - (p44_Y)));
			p12_Y = (p11_Y + (1.0f / 3.0f * (p14_Y - (p11_Y))));
			p13_Y = (p11_Y + (2.0f / 3.0f * (p14_Y - (p11_Y))));
			p23_Y = (p14_Y + (1.0f / 3.0f * (p41_Y - (p14_Y))));
			p32_Y = (p14_Y + (2.0f / 3.0f * (p41_Y - (p14_Y))));
			p21_Y = (p11_Y + (1.0f / 3.0f * (p41_Y - (p11_Y))));
			p31_Y = (p11_Y + (2.0f / 3.0f * (p41_Y - (p11_Y))));
			p42_Y = (p44_Y + (1.0f / 3.0f * (p41_Y - (p44_Y))));
			p43_Y = (p44_Y + (2.0f / 3.0f * (p41_Y - (p44_Y))));
...
void CMainApplication::keyDown(WORD keycode) {
	...
	ip.ki.wScan = MapVirtualKeyEx(keycode, MAPVK_VK_TO_VSC, GetKeyboardLayout(0));
	ip.ki.time = 0;
	ip.ki.dwExtraInfo = 0;
...
void CMainApplication::keyUp(WORD keycode) {
	//dprintf("\nKeyUp: %x", keycode);
	INPUT ip;
	ip.type = INPUT_KEYBOARD;
	ip.ki.wScan = MapVirtualKeyEx(keycode, MAPVK_VK_TO_VSC, GetKeyboardLayout(0)); 
...
//-----------------------------------------------------------------------------
// ################## OUR SOURCE CODE FOR STEPMANIA ENDS HERE ###################
//-----------------------------------------------------------------------------
	...
	for (vr::TrackedDeviceIndex_t unDevice = 0; unDevice < vr::k_unMaxTrackedDeviceCount; unDevice++)
	{
		...
		if (m_pHMD->GetControllerState(unDevice, &state, sizeof(state)))
		{
			...
			case vr::ETrackedDeviceClass::TrackedDeviceClass_HMD:
				...
				//printDevicePositionalData(unDevice, "HMD", poseMatrix, position, quaternion);
			...
			case vr::ETrackedDeviceClass::TrackedDeviceClass_Controller:
				...
				//	printDevicePositionalData(unDevice, whichHand.c_str(), poseMatrix, position, quaternion);
